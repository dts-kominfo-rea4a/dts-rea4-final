  ...
  return (
    <div className="container grid grid-cols-1 gap-4 p-6 mx-auto md:grid-cols-2 lg:grid-cols-3">
      {apps.map((app, idx) => (
        <div
          key={app.id}
          className="flex flex-col col-span-1 overflow-hidden bg-white rounded-t-lg rounded-b-lg shadow-lg dark:bg-gray-800 dark:border-gray-700 hover:shadow-sky-500 dark:hover:shadow-sky-700"
        >
...
const Apps = () => {
  ...
  //   [debouncedSearchTerm]
  // );
  const { isLoading, data } = useAppsQuery({});
